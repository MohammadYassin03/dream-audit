...
def hourly_wages_by_demographic() -> pd.DataFrame:
    ...
    Census P-38 (pre-2000) plus BLS LEU (2000+) weekly earnings."""
    wages = load_wages()
...
def time_price_table(long: pd.DataFrame) -> pd.DataFrame:
    ...
    # 02_clean refines once HUD Fair Market Rents are wired in.
    anchor_year = 2020
...
def persona_starter(long: pd.DataFrame, time_price: pd.DataFrame) -> pd.DataFrame:
    """The Starter (~22-27): how long to afford a year of college plus a year of rent."""
...
def persona_builder(long: pd.DataFrame, time_price: pd.DataFrame) -> pd.DataFrame:
    """The Builder (~35-45): home plus healthcare burden."""
...
def headline_stats(time_price: pd.DataFrame) -> dict:
    """Numbers for the article kickers. Written to data/processed/headline_stats.csv."""
    out = {}
